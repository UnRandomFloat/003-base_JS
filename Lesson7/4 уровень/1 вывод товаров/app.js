...
let buttonsVar = document.querySelectorAll("button");
let divPlate = document.querySelector(".products");
let propName = Object.keys(products);
let propValue = Object.values(products);

buttonsVar.forEach(element => element.addEventListener("click", clickHandler));
...
function showCategory(category) {
    for (let i = 0; i < propName.length; i++) {
        if (propName[i] === category) {
            propValue[i].forEach(element => getProductMarkup(element))
        }
    }
...
function getProductMarkup(product) {
    let adjastment = `<div class="product">
    <div>${product.name}</div>
    <img src="${product.imageUrl}" alt="">
    <div>${product.price}</div>
    <a href="https://example.com/producs/${product.id}">Подробнее</a>
    </div>`;
    divPlate.insertAdjacentHTML("beforeend", adjastment);
}
